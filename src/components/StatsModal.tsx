...
export default function StatsModal({ isVisible, toggleVisiblity }: Props) {
  ...
  return (
    <div className='absolute z-50 left-0 top-0 flex overflow-auto w-full h-screen justify-center bg-transparent backdrop-blur-sm' onClick={closeModal}>
      <div className={`${roboto.className} relative top-12 flex h-[700px] w-[600px] flex-col gap-2 rounded-md border bg-zinc-100 p-4 shadow-xl`} onClick={(e) => e.stopPropagation()}>
        <div className='mb-4 flex items-end justify-between border-b p-2 text-zinc-800'>
