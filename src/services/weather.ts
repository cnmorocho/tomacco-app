...
export async function getLatitudeAndLongitude(
  location: string
): Promise<Location[]> {
  const res: AxiosResponse<GeocodingApiResponse> = await axios.get(
    `https://geocoding-api.open-meteo.com/v1/search?name=${location.replace(' ', '+')}`
  );
...
export async function getWeather(latitude: string, longitude: string) {
  const res: AxiosResponse<Weather> = await axios.get(
